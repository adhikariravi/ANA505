...
total=mutate(df_hair_gender, total=cumsum(Freq))


#Since we have a females dataset, let's make a males dataset
#TASK: Write the function that includes only rows that are 'male'

df_hair_gender_male = df_hair_gender %>% filter(gender == 'Male')

#And now let's join the males and females
#TASK: Write the function that joins the male and female rows 
#(hint: try using 'union' or 'bind_rows')

# NOTE: Union removes duplicates. Union all does not.

df_merged = union_all(df_hair_gender_male, df_hair_gender_female)
